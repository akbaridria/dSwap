...
import { dSwapFactory, dSwapPair } from "../../typechain-types";
...
describe("dSwapFactory", () => {
  async function fixture() {
    const tmp = await ethers.getContractFactory("dSwapFactory");
  ...
  async function createPair(
    factory: dSwapFactory,
    tokens: [string, string],
  ) {
    const pairContract = await ethers.getContractFactory("dSwapPair");
    ...
    await expect(factory.createPair(tokens[0], tokens[1])).to.be.reverted; // dSwap: PAIR_EXISTS
    await expect(factory.createPair(tokens[1], tokens[0])).to.be.reverted; // dSwap: PAIR_EXISTS
    ...
    const pair = pairContract.attach(create2Address) as dSwapPair;
  ...
  it("Pair:codeHash", async () => {
    ...
    // const pair = await ethers.getContractFactory("dSwapPair");
  ...
  it("setFeeTo", async () => {
    ...
    ).to.be.revertedWith("dSwap: FORBIDDEN");
