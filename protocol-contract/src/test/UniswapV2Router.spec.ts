...
import { dSwapPair } from "../../typechain-types";

describe("dSwapRouter", () => {
  async function v2Fixture() {
    ...
    // deploy V2
    const v2factory = await ethers.getContractFactory("dSwapFactory");
    ...
    // deploy routers
    const router = await ethers.getContractFactory("dSwapRouter");
    ...
    const pairFactory = await ethers.getContractFactory("dSwapPair");
    const pair = (await pairFactory.attach(pairAddress)) as dSwapPair;
  ...
  it("quote", async () => {
    ...
    await expect(router.quote(0n, 100n, 200n)).to.be.revertedWith(
      "IDswapLibrary: INSUFFICIENT_AMOUNT",
    );
    await expect(router.quote(1n, 0n, 200n)).to.be.revertedWith(
      "IDswapLibrary: INSUFFICIENT_LIQUIDITY",
    );
  ...
  it("getAmountOut", async () => {
    ...
    await expect(router.getAmountOut(0n, 100n, 100n)).to.be.revertedWith(
      "IDswapLibrary: INSUFFICIENT_INPUT_AMOUNT",
    );
  ...
  it("getAmountIn", async () => {
    ...
    await expect(router.getAmountIn(0n, 100n, 100n)).to.be.revertedWith(
      "IDswapLibrary: INSUFFICIENT_OUTPUT_AMOUNT",
    );
  ...
  it("getAmountsOut", async () => {
    ...
    ).to.be.revertedWith("IDswapLibrary: INVALID_PATH");
